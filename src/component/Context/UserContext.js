...
export default function UserProvider({children}) {
    ...
    async function getUserData(){
        const {data} = await axios.get(`https://api.themoviedb.org/3/account?api_key=57309969f55d0b3d29084effa0d8081c&session_id=${session}`).finally(()=>setLoading(false))
        setUser(data)
    }
    ...
    return (
        <UserContext.Provider value={{user, Login, session, setUser, Loading, setLoading, favoriteList, setSearch, search, searchResults, setSearchResults, input, setInput }}>
            {children}
